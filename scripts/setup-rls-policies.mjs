/**
 * Check + fix RLS policies voor de klantenportaal
 */
...
// Use raw pg REST to check + apply RLS policies
...
// Step 1: Check current RLS status via information_schema
async function checkTables() {
  // Use supabase management API to check policies
  // We'll test by signing in as the user and checking what's visible
  
  const { data, error } = await supabase.auth.admin.getUserById('6a50eab0-ad68-483f-a444-dde2f252accb')
  if (error) { console.error('User lookup error:', error); return }
  
  console.log('User found:', data.user.email)
  
  // Test access as this user
  ...
  // We can't easily impersonate, so let's just create the SQL migration
  // and apply it via the known approach
}
...
console.log('═══════════════════════════════════════════════════════════')
console.log('RLS POLICIES SQL — Kopieer en voer uit in Supabase SQL Editor')
console.log('═══════════════════════════════════════════════════════════')
console.log('')
console.log(SQL_POLICIES)

// Also save as migration
const migrationPath = path.join(process.cwd(), 'supabase', 'migrations', '20260414_add_rls_policies.sql')
fs.writeFileSync(migrationPath, SQL_POLICIES.trim() + '\n')
console.log('')
console.log(`✓ SQL opgeslagen als: supabase/migrations/20260414_add_rls_policies.sql`)
console.log('')
console.log('Volgende stap: Kopieer bovenstaande SQL en voer uit in:')
console.log(`  ${url}/project/default/sql/new`)
