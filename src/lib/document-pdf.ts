...
export async function generatePDF(doc: DocumentData): Promise<jsPDF> {
  ...
  if (!isFac && doc.sign_link) {
    ...
    pdf.setFont('helvetica', 'bold')
    pdf.setFontSize(8)
    pdf.text('ONLINE GOEDKEUREN OF VRAAG STELLEN', ML + 8, y + 6.5)
    rule(pdf, P.border)
    pdf.setLineWidth(0.2)
    ...
    pdf.setFont('helvetica', 'normal')
    pdf.setFontSize(6.5)
    pdf.text('Scan om te reageren', ML + 6 + SQ / 2, y + 11 + SQ + 3.5, { align: 'center' })

    const lx = ML + SQ + 14
    ink(pdf, P.textSoft)
    pdf.setFontSize(8)
    pdf.text('Scan de QR-code of klik op de link om de offerte online', lx, y + 14)
    pdf.text('te bekijken, een vraag te stellen of digitaal te ondertekenen.', lx, y + 19)
    ink(pdf, P.orange)
    pdf.setFont('helvetica', 'bold')
    pdf.setFontSize(7.5)
