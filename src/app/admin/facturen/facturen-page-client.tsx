...
export default function FacturenPageClient({ facturen }: { facturen: Factuur[] }) {
  ...
  const openBedrag = facturen.filter((f) => f.status === 'verstuurd').reduce((sum, f) => sum + (f.total || 0), 0)
              ...
              {facturen.map((f) => (
                ...
                >
                  ...
                  <div className="shrink-0 text-right">
                    <p className="text-sm font-bold text-[var(--text-main)]">€{(f.total || 0).toFixed(2)}</p>
                    <p className="text-[10px] text-[var(--text-muted)]">
                      {f.created_at ? new Date(f.created_at).toLocaleDateString('nl-BE') : ''}
                    </p>
                  </div>
                </Link>
              ))}
